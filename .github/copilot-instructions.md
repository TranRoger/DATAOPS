...
- **Source freshness checks** configured with `warn_after` and `error_after` thresholds
  - Run freshness checks: `docker-compose exec dbt dbt source freshness`
...
### Airflow DAG Pattern
- **Production DAG**: `airflow/dags/dbt_pipeline_enhanced.py` (recommended)
- **Legacy DAG**: `airflow/dags/dbt_dag.py` (basic version)
- Uses BashOperator to execute `docker exec` commands (NOT DockerOperator)
- Container name: `dbt_airflow_project-dbt-1` (generated by docker-compose, not just `dbt`)
- **Enhanced DAG Features**:
  - Full medallion architecture: Bronze → Silver → Gold
  - Source freshness checks before processing
  - Data quality tests between each layer
  - Comprehensive error handling with 2 retries and exponential backoff
  - Success and failure notifications
  - Documentation generation
  - Schedule: Daily at 2 AM UTC (`0 2 * * *`)
- Task dependencies: `check_source_freshness >> dbt_run_bronze >> dbt_test_bronze >> dbt_run_silver >> dbt_test_silver >> dbt_run_gold >> dbt_test_gold >> dbt_generate_docs >> notify_success`
- See `airflow/DAG_DOCUMENTATION.md` for complete usage guide
...
- Connection config is in `dbt/profiles.yml` (NOT environment variables)
  - **Note**: `profiles.yml` is NOT tracked in git (in `.gitignore`)
  - Created during container build or manually
  - Located at `/usr/app/dbt/profiles.yml` inside container
  - Connection string uses: `host.docker.internal:1433` or `sqlserver` (container name)
